...
angular.module('libraryManagementSystem')
  .controller('returnBookCtrl', function ($scope,Book,User) {
/**fetch bookdetails************/
 getBook();
 var bookID =   "";
...
if(bookID==""){
  alert("please select the book to return");
  return;
}
...
xhttp.open("POST", "http://localhost:3000/return", false);
...
function getBook(){
 ...
 $scope.books = data.filter(function(data){
  ...
  if(data.availability=="N"){return true}
  return false; 
 });//filter
